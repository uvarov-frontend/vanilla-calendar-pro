...
- `tests/browser/native.mjs`: six checks using real mouse/keyboard input, including Enter, date clicks before popup positioning and interactive popup content, pointer capture outside the calendar, collapse/expand, and interruption by `set()`/`destroy()`. Full Chrome/WebKit runs execute these after Cypress. Cypress gesture specs use a scoped capture shim for synthetic pointers, which browsers do not consistently register as active hardware pointers; these native checks do not use the shim.
