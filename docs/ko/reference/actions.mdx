...
ESM 마이그레이션: 기존 옵션과 콜백을 유지하고 `vanilla-calendar-pro`에서 `annotations` 확장을 import한 뒤 생성자의 `extensions: [annotations]` 배열에 추가하세요. 함수처럼 호출하지 않습니다.

이 API는 HTML을 받습니다. 신뢰할 수 있는 마크업을 사용하거나, 사용자 또는 신뢰할 수 없는 출처의 콘텐츠에는 [`sanitizerHTML`](/ko/docs/reference/settings#sanitizerhtml)을 설정하세요. 기본 콜백은 HTML을 정제하지 않습니다.
