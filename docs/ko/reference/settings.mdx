...
`motion`은 애니메이션, 스와이프 탐색과 월/주 접기를 제공합니다. `timePicker`는 12/24시간 시간 선택기를 제공합니다. `datePopups`는 날짜 범위를 포함한 날짜 팝업 내용과 스타일 modifier를 제공합니다. ESM 번들러는 import하고 등록하지 않은 확장 기능을 제거할 수 있습니다.
...
입력 필드 캘린더, 날짜 및 범위 선택, 월/연도 선택, 주간 보기와 범위 툴팁은 코어에 유지됩니다. 일반 `<script>` / CommonJS 전체 빌드는 세 확장 기능을 자동 등록합니다. CSS import는 바뀌지 않습니다.
...
ESM: 생성자의 `extensions` 배열에 `timePicker`을 등록해야 합니다. 이 옵션과 값은 기존 Calendar 설정에 그대로 둡니다.
