...
ESM에서 `type: 'multiple'`에는 `months`, `type: 'week'`에는 `weeks`가 필요합니다. 나중에 `set()`으로 전환하는 경우에도 생성자에서 미리 등록하세요.
...
확장 기능이 없는 이전 버전에서 업그레이드할 때 기존 옵션과 콜백은 유지하세요. ESM에서는 `motion`을 이름으로 import하고 생성자에 `extensions: [motion]`을 추가하세요. 확장 기능을 함수처럼 호출하지 않습니다. import하거나 등록하는 것만으로 기능이 활성화되지는 않습니다.
...
ESM 마이그레이션: 기존 옵션과 콜백을 유지하고 `vanilla-calendar-pro`에서 `months` 확장을 import한 뒤 생성자의 `extensions: [months]` 배열에 추가하세요. 함수처럼 호출하지 않습니다.
...
확장 기능이 없는 이전 버전에서 업그레이드할 때 기존 옵션과 콜백은 유지하세요. ESM에서는 `weeks` 확장을 이름으로 import하고 생성자에 `extensions: [weeks]`을 추가하세요. 확장 기능을 함수처럼 호출하지 않습니다. import하거나 등록하는 것만으로 기능이 활성화되지는 않습니다.
...
접기와 펼치기는 `weeks`가 담당합니다. `motion` 없이도 버튼을 누르면 즉시 전환됩니다. 드래그에는 `weeks`와 `motion`을 함께 등록하고, 전환 애니메이션에는 `animation`을 활성화하세요. `motion`만으로는 주간 보기가 포함되지 않습니다.
...
확장 기능이 없는 이전 버전에서 업그레이드할 때 기존 옵션과 콜백은 유지하세요. ESM에서는 `time`을 이름으로 import하고 생성자에 `extensions: [time]`을 추가하세요. 확장 기능을 함수처럼 호출하지 않습니다. import하거나 등록하는 것만으로 기능이 활성화되지는 않습니다.
...
ESM: 생성자의 `extensions` 배열에 `time`을 등록해야 합니다. 이 옵션과 값은 기존 Calendar 설정에 그대로 둡니다.
...
`sanitizerHTML`은 `layouts`, `popups.html`, `onCreateDateRangeTooltip`에서 제공하는 HTML을 처리합니다. 기본 구현은 문자열을 그대로 반환하며 캘린더에는 HTML 정제기가 내장되어 있지 않습니다. 신뢰할 수 있는 마크업만 사용하거나 외부 콘텐츠를 렌더링하기 전에 DOMPurify처럼 유지 관리되는 정제기를 설정하세요. 이 콜백은 앱의 Content Security Policy를 설정하지 않습니다. `labels`와 `styles`는 HTML이 아닌 일반 텍스트와 CSS 클래스 이름으로 처리됩니다.
