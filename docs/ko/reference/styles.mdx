...
각 그룹에는 `core`, `motion`, `time`, `annotations`, `weeks`, `months`가 있습니다. `core`와 사용하는 모든 확장의 스타일을 가져오세요. 나중에 `set()`으로 활성화할 기능도 포함해야 합니다.

| 경로 | 내용 |
| --- | --- |
| `styles/index.css` | 전체 레이아웃과 밝은/어두운 테마 |
| `styles/{part}.css` | 선택한 부분의 레이아웃과 밝은/어두운 테마 |
| `styles/layout/{part}.css` | 선택한 부분의 레이아웃만 포함, 테마 제외 |
| `styles/themes/{theme}/{part}.css` | 선택한 테마에서 해당 부분의 스타일만 포함 |

시간 선택과 자동 밝은/어두운 테마 전환을 사용하는 달력:
...
테마를 별도로 선택하려면 레이아웃 부분과 해당 테마 부분을 가져오세요. 시간 선택을 사용하는 밝은 달력의 예:
...
각 계층에서 전체 파일 또는 모듈 파일 중 하나를 선택하여 규칙이 중복 로드되지 않게 하세요. CSS 클래스, 선택자, 브라우저 지원과 공개 `--vc-*` 재정의는 유지됩니다. `motion`, `months`와 같이 테마별 규칙이 없는 부분은 유효한 빈 CSS 파일입니다.

모든 확장을 사용한다면 전체 스타일시트를 유지하세요. 여섯 부분을 모두 가져오는 것보다 압축 효율이 좋습니다.

다운로드 가능한 `package.zip`에는 다섯 개의 전체 스타일시트가 포함됩니다. 모듈별 CSS는 npm 패키지와 직접 CDN URL로 제공됩니다.
...
캘린더는 거의 모든 크기를 `rem` 단위로 지정하므로 루트 폰트 크기(root font-size)에 따라 함께 스케일됩니다. 페이지에서 이를 브라우저 기본값인 `16px`에서 다른 값으로 재설정한 경우(오래된 프레임워크인 Bootstrap 3 등에서 흔하며, 보통 `10px`), 캘린더가 잘못된 비율로 렌더링됩니다. `transform` 하나로 원래 크기를 복원할 수 있습니다:

```css
[data-vc='calendar'] {
  transform: scale(16 / <루트 폰트 크기, px 단위>);
}
```

예를 들어 루트가 `10px`인 경우: `transform: scale(1.6);`.
