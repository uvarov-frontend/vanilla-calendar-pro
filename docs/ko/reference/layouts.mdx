...
`<#ControlTime />`은 `timePicker` 등록 후 `selectionTimeMode`를 활성화해야 렌더링됩니다. `<#Collapse />`은 `motion` 등록 후 `enableCollapse: true`로 설정해야 렌더링됩니다. 템플릿 태그는 유지하세요. 등록만으로 해당 옵션이 활성화되지는 않습니다.
