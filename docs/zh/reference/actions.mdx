...
ESM 迁移：保留现有选项和回调，从 `vanilla-calendar-pro` 导入 `annotations`，并加入构造函数的 `extensions: [annotations]` 数组，无需调用扩展函数。

此 API 接受 HTML。请使用可信标记；如果内容来自用户或其他不可信来源，请配置 [`sanitizerHTML`](/zh/docs/reference/settings#sanitizerhtml)。默认回调不会清理 HTML。
