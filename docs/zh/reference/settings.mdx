...
使用 ESM 时，`type: 'multiple'` 需要 `months`，`type: 'week'` 需要 `weeks`。即使以后才通过 `set()` 切换，也应在构造函数中注册扩展。
...
从没有扩展机制的版本升级时，请保留现有选项和回调。使用 ESM 时，添加 `motion` 的具名导入，并在构造函数中配置 `extensions: [motion]`。无需调用扩展函数。仅导入或注册扩展不会自动启用功能。
...
ESM 迁移：保留现有选项和回调，从 `vanilla-calendar-pro` 导入 `months`，并加入构造函数的 `extensions: [months]` 数组，无需调用扩展函数。
...
从没有扩展机制的版本升级时，请保留现有选项和回调。使用 ESM 时，添加 `weeks` 的具名导入，并在构造函数中配置 `extensions: [weeks]`。无需调用扩展函数。仅导入或注册扩展不会自动启用功能。
...
折叠和展开由 `weeks` 提供；不使用 `motion` 时，点击按钮会立即切换视图。拖动需要同时注册 `weeks` 和 `motion`；启用 `animation` 可添加过渡动画。单独使用 `motion` 不会包含一周视图。
...
从没有扩展机制的版本升级时，请保留现有选项和回调。使用 ESM 时，添加 `time` 的具名导入，并在构造函数中配置 `extensions: [time]`。无需调用扩展函数。仅导入或注册扩展不会自动启用功能。
...
ESM：需要在构造函数的 `extensions` 数组中注册 `time`；此选项及其值仍保留在 Calendar 配置中。
...
`sanitizerHTML` 处理来自 `layouts`、`popups.html` 和 `onCreateDateRangeTooltip` 的 HTML。默认实现原样返回字符串，日历不内置 HTML 清理器。请仅使用可信标记，或在渲染外部内容前配置持续维护的清理器，例如 DOMPurify。此回调不会配置应用的 Content Security Policy。`labels` 和 `styles` 分别作为普通文本和 CSS 类名处理，而不是 HTML。
