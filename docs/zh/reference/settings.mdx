...
`motion` 包含动画、滑动导航和整月与一周之间的折叠。`timePicker` 提供 12/24 小时时间选择。`datePopups` 提供日期弹出内容和样式修饰符，也支持日期范围。ESM 打包工具可移除未导入和注册的扩展。
...
输入框日历、日期和范围选择、月份和年份选择、一周视图以及范围提示仍属于核心功能。完整的普通 `<script>` / CommonJS 构建会自动注册全部三个扩展。CSS 导入保持不变。
...
ESM：需要在构造函数的 `extensions` 数组中注册 `timePicker`；此选项及其值仍保留在 Calendar 配置中。
