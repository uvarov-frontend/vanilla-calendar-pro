...
`<#ControlTime />` 需要 `time` 和已启用的 `selectionTimeMode`。`<#Collapse />` 需要 `weeks` 和 `enableCollapse: true`；`<#WeekNumbers />` 需要 `weeks` 和 `enableWeekNumbers: true`。`<#DateRangeTooltip />` 需要 `annotations` 和 `onCreateDateRangeTooltip`。`<#Multiple>` 需要 `months`。请保留原有模板标签和选项。
