...
ESM migration: keep the existing options and callbacks, import `annotations` from `vanilla-calendar-pro`, and add it to the constructor’s `extensions: [annotations]` array without calling it.

This API accepts HTML. Use trusted markup, or configure [`sanitizerHTML`](/docs/reference/settings#sanitizerhtml) when content comes from users or other untrusted sources; the default callback does not sanitize it.
