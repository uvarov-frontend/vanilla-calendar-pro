...
<Info>This option has no effect when `selected.month` and `selected.year` are provided.</Info>
...
<Info>
  Please note that the external library{' '}
  <a href="https://www.npmjs.com/package/dompurify" target="_blank" rel="nofollow noreferrer">
    `dompurify`
  </a>{' '}
  is used as an example. Since `sanitizer` is not required for the calendar to function, using an external library helps avoid increasing the calendar's code
  size.
</Info>
