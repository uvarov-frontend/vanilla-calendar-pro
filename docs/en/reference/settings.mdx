...
For ESM, `type: 'multiple'` requires `months`; `type: 'week'` requires `weeks`. Register the module in the constructor, including when switching to that view later with `set()`.
...
ESM migration: keep the existing options and callbacks, import `months` from `vanilla-calendar-pro`, and add it to the constructor’s `extensions: [months]` array without calling it.
...
When upgrading from a version without extensions, keep your existing options and callbacks. For ESM, add the named import and register `time` in the constructor with `extensions: [time]`. Do not call the extension. Importing or registering it does not enable the feature by itself.
...
ESM: requires `time` in the constructor’s `extensions` array; keep this option and its value in the Calendar configuration.
...
`sanitizerHTML` processes HTML from `layouts`, `popups.html` and `onCreateDateRangeTooltip`. Its default implementation returns the string unchanged: the calendar does not include an HTML sanitizer. Use only trusted markup, or configure a maintained sanitizer such as DOMPurify before rendering external content. This hook does not configure your Content Security Policy. `labels` and `styles` are treated as literal text and CSS class names, not HTML.
