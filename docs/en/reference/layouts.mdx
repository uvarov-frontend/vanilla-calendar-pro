...
`<#ControlTime />` renders only with `timePicker` registered and `selectionTimeMode` enabled. `<#Collapse />` renders only with `motion` registered and `enableCollapse: true`. Keep these template tags; registration does not enable the corresponding options.
