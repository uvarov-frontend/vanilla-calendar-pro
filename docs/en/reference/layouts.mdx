...
`<#ControlTime />` requires `time` and an enabled `selectionTimeMode`. `<#Collapse />` requires `weeks` and `enableCollapse: true`; `<#WeekNumbers />` requires `weeks` and `enableWeekNumbers: true`. `<#DateRangeTooltip />` requires `annotations` and `onCreateDateRangeTooltip`. `<#Multiple>` requires `months`. Keep the template tags and existing options.
