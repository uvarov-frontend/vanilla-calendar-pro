...
`Type: readonly CalendarExtension[]`

`Default: []`

`motion` предоставляет анимации и жесты. `time` отвечает за выбор времени в формате 12/24 часа. `annotations` добавляет попапы, модификаторы дат и тултип диапазона. `weeks` предоставляет недельный вид, сворачивание, номера недель и обработчики кликов по дням и номерам недель. `months` отображает несколько месяцев и управляет их навигацией. Сборщик ESM может удалить расширения, которые вы не импортируете и не подключаете.
...
Календарь в инпуте, выбор дат и диапазонов, выбор месяца и года остаются в ядре. Обычные заголовки дней недели не требуют расширения. Полная сборка для обычного `<script>` / CommonJS автоматически включает все пять расширений. Импорты CSS не меняются.

---

## type

Для ESM режим `type: 'multiple'` требует `months`, а `type: 'week'` — `weeks`. Зарегистрируйте модуль в конструкторе, в том числе при последующем переключении вида через `set()`.
...
Миграция ESM: сохраните существующие опции и обработчики, импортируйте `months` из `vanilla-calendar-pro` и добавьте его в массив `extensions: [months]` конструктора без вызова как функции.
...
При обновлении с версии без расширений сохраните прежние опции и обработчики. Для ESM добавьте именованный импорт и зарегистрируйте `time` в конструкторе: `extensions: [time]`. Вызывать расширение как функцию не нужно. Импорт или регистрация сами по себе не включают функцию.
...
ESM: требуется `time` в массиве `extensions` конструктора; сама опция и её значение остаются в конфигурации Calendar.
...
`sanitizerHTML` обрабатывает HTML из `layouts`, `popups.html` и `onCreateDateRangeTooltip`. По умолчанию функция возвращает строку без изменений: встроенного HTML-санитайзера в календаре нет. Используйте доверенную разметку либо подключите поддерживаемый санитайзер, например DOMPurify, перед выводом внешнего содержимого. Этот обработчик не настраивает Content Security Policy приложения. `labels` и `styles` интерпретируются как обычный текст и имена CSS-классов, а не HTML.
