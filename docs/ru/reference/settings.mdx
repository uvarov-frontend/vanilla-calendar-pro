...
Анимирует переходы между представлениями: переключение месяцев стрелками и листание списка годов едут по горизонтали, открытие и закрытие выбора месяца и года идут кроссфейдом.

<Info>Даже при включённой опции потеря производительности незаметна: переход добавляет около 2ms разовой работы (не на каждый кадр), а сама анимация идёт на композиторе — анимируются только `transform` и `opacity`, поэтому основной поток не блокируется. К размеру бандла опция добавляет примерно 1.5 KB (в gzip).</Info>

Значения по умолчанию разные для разных переходов — `250ms` для слайдов и `150ms` для кроссфейда, оба с `cubic-bezier(0.4, 0, 0.2, 1)`. Объект переопределяет любое из значений: `duration` указывается в миллисекундах, `easing` принимает любую CSS-функцию плавности. Значения верхнего уровня действуют на все переходы; вложите их в `slide` (стрелки) или `fade` (пикеры), чтобы задать одну группу — вложенные важнее.

<Info>Анимация не проигрывается, если посетитель запросил уменьшение движения через `prefers-reduced-motion: reduce`.</Info>

По умолчанию `false` ради обратной совместимости: опция появилась позже, а её включение меняет то, что видит поиск по DOM. Пока идёт переход, уходящее содержимое остаётся в DOM внутри слоя `[data-vc-ghost]` с атрибутом `inert`, поэтому элементы дат на это время присутствуют дважды — исключайте этот слой, если ваш код их ищет, иначе `mainElement.querySelectorAll('[data-vc-date]')` вернёт ещё и ячейки, которые уже уезжают.
...
<Info>Если `monthsToSwitch` больше `1`, то в режиме выбора месяца (month picker) будут доступны для выбора только те месяцы, которые достижимы от текущего выбранного месяца с шагом `monthsToSwitch` — остальные будут отображаться отключёнными. Это нужно для согласованности навигации с заданным шагом (особенно актуально вместе с `displayMonthsCount` при `type: 'multiple'`, где это удерживает несколько видимых месяцев синхронизированными).</Info>
...
<Info>Передача `null` в `displayDateMin` через `.set()` явно сбрасывает значение обратно к дефолтному. Передача `undefined` (например, отсутствие этого свойства) оставляет текущее значение без изменений.</Info>
...
<Info>Передача `null` в `displayDateMax` через `.set()` явно сбрасывает значение обратно к дефолтному. Передача `undefined` (например, отсутствие этого свойства) оставляет текущее значение без изменений.</Info>
