...
В каждой группе доступны `core`, `motion`, `time`, `annotations`, `weeks` и `months`. Подключите `core` и стили всех используемых расширений, включая функции, которые будут включены позднее через `set()`.

| Путь | Содержимое |
| --- | --- |
| `styles/index.css` | Вся раскладка и светлая/тёмная темы |
| `styles/{part}.css` | Выбранная часть: раскладка и светлое/тёмное оформление |
| `styles/layout/{part}.css` | Только выбранная часть раскладки, без темы |
| `styles/themes/{theme}/{part}.css` | Только выбранная часть одной темы |

Календарь с выбором времени и автоматическим переключением светлой/тёмной темы:
...
Для независимого выбора темы подключите части раскладки и соответствующие части темы. Например, светлый календарь с выбором времени:
...
В каждом слое выбирайте полные файлы либо отдельные части, чтобы не загружать одни правила дважды. CSS-классы, селекторы, поддержка браузеров и публичные переопределения `--vc-*` сохраняются. Некоторые части тем, например `motion` и `months`, не содержат собственных цветовых правил и являются корректными пустыми CSS-файлами.

Если используются все расширения, оставьте полный файл стилей: он сжимается лучше, чем все шесть частей вместе.

Архив `package.zip` содержит пять полных файлов стилей. Модульные части доступны в npm-пакете и по прямым CDN-ссылкам.
...
Все размеры календаря заданы в `rem`, поэтому он масштабируется вместе с базовым размером шрифта (root font-size). Если на странице этот размер сброшен со стандартных браузерных `16px` (частая практика на старых фреймворках вроде Bootstrap 3, обычно до `10px`), календарь отрендерится в неверном масштабе. Вернуть исходный размер можно одним `transform`:

```css
[data-vc='calendar'] {
  transform: scale(16 / <root font-size в px>);
}
```

Например, при `10px` на корне: `transform: scale(1.6);`.
