...
Свойство `userTime` указывает на то, как было установлено время при инициализации календаря: автоматическое определение текущего времени (по умолчанию) или использование параметра `settings.selected.time`.
