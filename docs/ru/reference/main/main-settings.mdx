...
<Info>Эта опция не имеет эффекта, если указаны `selected.month` и `selected.year`.</Info>
...
<Info>
  ...
  . Поскольку `sanitizer` не является обязательным для работы календаря, подключение сторонней библиотеки помогает избежать увеличения кода календаря.
</Info>
