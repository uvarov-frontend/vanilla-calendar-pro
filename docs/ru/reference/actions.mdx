...
Миграция ESM: сохраните существующие опции и обработчики, импортируйте `annotations` из `vanilla-calendar-pro` и добавьте его в массив `extensions: [annotations]` конструктора без вызова как функции.

Этот API принимает HTML. Используйте доверенную разметку либо настройте [`sanitizerHTML`](/ru/docs/reference/settings#sanitizerhtml), если содержимое поступает от пользователей или из других недоверенных источников; по умолчанию обработчик ничего не очищает.
