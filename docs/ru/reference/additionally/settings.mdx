...
<Info>
  Важно отметить, что `settings.range.min` и `settings.range.max` отключают даты, выходящие за пределы диапазона, в то время как `date.min` и `date.max` вообще
  их не создают.
</Info>

<Info>Вы можете указать `'today'` как ярлык для `settings.range.min` и/или `settings.range.max`.</Info>
...
<Info>
  Важно отметить, что при использовании этой настройки отключение дат в пределах диапазона дат не будет иметь никакого эффекта. Поэтому обязательно используйте
  этот параметр, если вас интересуют только начальная и конечная выбранная пользователем дата.
</Info>
