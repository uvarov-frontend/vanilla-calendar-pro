...
- `e` - событие;
- `calendar` - объект, содержащий методы `show()`, `hide()`;
- `self` - ссылка на инициализированный календарь.
...
Этот метод срабатывает, когда тип календаря установлен на `'year'`. Тип календаря становится `'year'`, когда пользователь кликает на заголовок года или при инициализации с параметром `type = 'year'`. Он предоставляет доступ к информации о каждом месяце и его HTML-элементе. Вы можете получить следующие параметры:
- `year` - номер года;
