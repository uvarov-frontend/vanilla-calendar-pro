...
`<#ControlTime />` выводится при подключённом `timePicker` и включённой `selectionTimeMode`. `<#Collapse />` выводится при подключённом `motion` и `enableCollapse: true`. Сами теги шаблона менять не нужно; регистрация не включает соответствующие опции.
