...
`<#ControlTime />` требует `time` и включённую `selectionTimeMode`. `<#Collapse />` требует `weeks` и `enableCollapse: true`, а `<#WeekNumbers />` — `weeks` и `enableWeekNumbers: true`. `<#DateRangeTooltip />` требует `annotations` и `onCreateDateRangeTooltip`. `<#Multiple>` требует `months`. Теги шаблона и существующие опции сохраняются.
