describe('Init default calendar', () => {
  ...
  it('Check months', () => {
    ...
    cy.get('#calendar')
      ...
      .should('have.attr', 'data-calendar-selected-month');
    cy.get('#calendar').find('.vanilla-calendar-month').click();
    cy.get('#calendar')
      .find('.vanilla-calendar-months')
      .children('.vanilla-calendar-months__month')
      .then(($month) => $month[1].click());
    cy.get('#calendar')
      .find('.vanilla-calendar-month')
      .should('have.attr', 'data-calendar-selected-month', '1');
  });
  it('Check years', () => {
    cy.visit('/');
    cy.get('#calendar')
      .find('.vanilla-calendar-year')
      .should('have.attr', 'data-calendar-selected-year');
    cy.get('#calendar').find('.vanilla-calendar-year').click();
    cy.get('#calendar')
      .find('.vanilla-calendar-years')
      .children('.vanilla-calendar-years__year[data-calendar-year="2022"]')
      .click();
    cy.get('#calendar')
      .find('.vanilla-calendar-year')
  ...
  it('Check days', () => {
    ...
    cy.get('#calendar')
      ...
      .should('have.attr', 'data-calendar-day', '2023-04-06');
  });
});
