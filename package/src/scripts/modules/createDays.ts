...
const createDay = (
	...
	dayBtnEl.dataset.vcDate = date;

	const addWeekNumber = () => {
		const weekNumber = getWeekNumber(date, self.settings.iso8601);
		if (!weekNumber) return;
		dayBtnEl.dataset.vcWeekNumber = String(weekNumber.week);
	};
...
const createDays = (self: VanillaCalendar) => {
	const daysEls = self.HTMLElement.querySelectorAll<HTMLElement>('[data-vc="days"]');
	...
	daysEls.forEach((daysEl, index: number) => {
		...
		if (!self.settings.selection.day) daysEl.ariaDisabled = '';
		daysEl.textContent = '';
